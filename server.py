...
api_keys = [
    'AIzaSyDHeGHEL6kbLwP76W0x1MeYs7KbFj-xI0g',
    'AIzaSyBenzEtCwa_jddG5kOGH2FgSyh_RmlelPs',
    'AIzaSyBbUxzdsTigSAcuCcQ4wFMUyq_PayWmU5E'
...
def interact_with_gemini(prompt, history=[], max_retries=3, retry_delay=2):
    for api_key in api_keys:
        ...
        for attempt in range(max_retries):
            try:
                ...
                return text_response  # Trả về câu trả lời nếu thành công

            except google_exceptions.TooManyRequests as e:
                print(f"Lỗi quá nhiều yêu cầu với API key {api_key}. Thử lại sau {retry_delay} giây...")
                time.sleep(retry_delay)  # Chờ một khoảng thời gian trước khi thử lại
                continue  # Thử lại với cùng một API key

            except google_exceptions.InternalServerError as e:
                print(f"Lỗi máy chủ nội bộ với API key {api_key}. Thử lại sau {retry_delay} giây...")
                time.sleep(retry_delay)  # Chờ một khoảng thời gian trước khi thử lại
                continue  # Thử lại với cùng một API key

            except Exception as e:
                print(f"Lỗi không xác định với API key {api_key}: {str(e)}")
                break  # Ngừng thử với API key này và chuyển sang key khác

    print("Tất cả các API key đều gặp lỗi. Vui lòng thử lại sau.")
    return None  # Trả về None nếu tất cả các API key đều gặp lỗi
